 ...
 ******************************************************************************/
ThdResult ThreadLock::wait (double seconds)
{
	int result = 0;
 ...
 ******************************************************************************/
ThdResult ThreadLock::broadcast ()
{
	lock ();
	/* Initialize condition variable, if not yet initialized. */
	if (!mCondInited)
	...
	)
{
	...
	if (result)
		return MERR_THREAD_JOIN_FAILED;
